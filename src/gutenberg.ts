...
function openOpf(zip: AdmZip) {
  ...
  const opfDir = opfPath.includes("/")
    ...
    : "";
  return { opf: cheerio.load(readEntry(opfPath), { xml: true }), opfDir, readEntry };
}
...
export function epubToAnchored(data: Buffer): GutenbergBook {
  ...
  opf("spine > itemref").each((_, el) => {
    ...
    const page = cheerio.load(readEntry(full));
    ...
    page("body")
      ...
      .each((_i, node) => {
        ...
        children.each((_j, child) => {
          const html = (page.html(child) ?? "").trim();
          if (html && !PG_MARKER.test(page(child).text())) {
            blocks.push(html);
          }
        });
      });
  ...
  let current: Section = { title: null, html: [] };
  // Entities decode to spaces so a heading like "STAVE&nbsp;TWO" slugs to
  // "stave-two", not "stave-nbsp-two".
  const headingText = (html: string): string | null => {
  ...
  for (const html of blocks) {
    ...
    if (heading !== null) {
      ...
      current = { title: heading, html: [html] };
    } else {
  ...
  for (const [i, s] of kept.entries()) {
    ...
    section("p").each((_, p) => {
      ...
      if (!section(p).attr("id")) {
        section(p).attr("id", `${id}-p${n}`);
      }
    });
    out.push(section("section").prop("outerHTML") ?? "");
  }
  ...
  return { title, author, language, body };
}
