...
import type { WorkerOut } from "./core.ts";
 ...
 *  silently clobbering someone's data is worse than asking them to look. */
export async function loadDoc(dir: string): Promise<Doc> {
  ...
  if (doc.format !== FORMAT) {
    throw new Error(`${file}: unknown format "${doc.format}" (expected ${FORMAT})`);
  }
  return doc;
}
